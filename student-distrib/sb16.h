...
int8_t DSP_reset();
int8_t DSP_write(uint8_t data);
uint8_t Read_From_DSP();
int8_t Transfer_Sound_DMA(uint8_t channel, uint8_t mode, uint32_t addr, uint32_t size);
int8_t Rate_set(uint16_t frequency);
extern void Sound_player_init();
void Sound_player_handler();
void play_start(uint32_t block_size);
void play_stop();
extern int8_t sound_play(int8_t* filename);
...
extern int8_t play_music(int8_t* filename);
