...
static int8_t sound_filename[33];
static uint32_t current_offset;
static int8_t is_playing = 0;
static uint32_t sound_file_inode = 0;

static int8_t ports[8] = {0x87, 0x83, 0x81, 0x82, 0x00, 0x8B, 0x89, 0x8A};

void Sound_player_init(){
    enable_irq(SB16_IRQ);
}

void DSP_outb(uint8_t data, uint8_t offset){
    outb(data, SB16_IOBase + offset);
}

uint8_t DSP_inb(uint8_t offset){
    return inb(SB16_IOBase + offset);
}
...
int8_t DSP_reset(){
    ...
    uint8_t i = 0;
    uint8_t read;
    outb(1, SB16_IOBase + DSP_Reset);
    while(i < _3ms){i++;}
    outb(0, SB16_IOBase + DSP_Reset);

    while(1){
        read = inb(SB16_IOBase + DSP_Read_Buffer_Status);
        if( (read & (1 << 7)) == 1 )
            break;
    }

    while(1){
        read = inb(SB16_IOBase + DSP_Read);
        if( (read & 0xAA) == 1)
            break;
    }
